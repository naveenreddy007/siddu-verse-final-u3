...
interface TournamentPulseFeedProps {
  posts: PulsePost[]
  title?: string
}

export default function TournamentPulseFeed({ posts, title = "Tournament Pulse" }: TournamentPulseFeedProps) {
  if (posts.length === 0) {
    return (
          ...
          >
            ...
            {post.tags && post.tags.length > 0 && (
              <div className="flex flex-wrap gap-2 mb-4">
                {post.tags.map((tag, i) => (
                  <span key={i} className="bg-[#333] text-[#00BFFF] text-xs px-2 py-1 rounded-full">
                    #{tag}
                  </span>
                ))}
              </div>
